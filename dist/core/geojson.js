...
export class GeoJSONProcessor {
     ...
     */
    static async load(countryCode, system = 'WGS84') {
        ...
        return await JsonStore.read(filePath);
    }
    /**
     * 保存指定国家的GeoJSON数据
     ...
     * @param system 坐标系类型，默认WGS84
     */
    static async save(countryCode, geojson, system = 'WGS84') {
        const fileName = this.getGeoJsonFileName(countryCode, system);
        const filePath = JsonStore.getDistPath(fileName);
     ...
     */
    static async merge(countryCodes, system = 'WGS84') {
     ...
     */
    static async transformAndSave(countryCode, from, to) {
        ...
        logger.info(`Transformed and saved GeoJSON for ${countryCode}: ` +
     ...
     * @returns 生成的版本数量
     */
    static async generateSparseVersions(countryCode, distances) {
     ...
     * @returns 国家代码数组
     */
    static async getAvailableCountryCodes(system = 'WGS84') {
     ...
     * @returns 处理结果统计
     */
    static async processAllCountries(distances) {
        ...
        logger.info(`Processing complete: ${result.success} succeeded, ${result.failed} failed, ` +
            `${result.total} total`);
        return result;
    }
    /**
     ...
     */
    static getGeoJsonFileName(countryCode, system) {
     ...
     * @returns 相对于dist目录的文件路径
     */
    static getSparseFileName(countryCode, distance) {
     ...
     * @returns 原始GeoJSON数据
     */
    static async loadOriginal(countryCode) {
        const filePath = JsonStore.getDistPath(path.join(this.PATHS.original, `${countryCode}.geojson`));
     ...
     * @param geojson GeoJSON数据
     */
    static async saveOriginal(countryCode, geojson) {
        const filePath = JsonStore.getDistPath(path.join(this.PATHS.original, `${countryCode}.geojson`));
     ...
     */
    static async loadSparse(countryCode, distance) {
        const fileName = this.getSparseFileName(countryCode, distance);
        const filePath = JsonStore.getDistPath(fileName);
     ...
     * @returns 是否存在
     */
    static async exists(countryCode, system = 'WGS84') {
        const fileName = this.getGeoJsonFileName(countryCode, system);
        const filePath = JsonStore.getDistPath(fileName);
     ...
     * @returns 是否成功删除
     */
    static async delete(countryCode, system = 'WGS84') {
        const fileName = this.getGeoJsonFileName(countryCode, system);
        const filePath = JsonStore.getDistPath(fileName);
