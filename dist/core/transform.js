...
export class CoordinateTransformer {
     ...
     */
    static convert(lng, lat, from, to) {
        // 相同坐标系无需转换
        if (from === to) {
            return [lng, lat];
        }
     ...
     */
    static transformGeoJSON(geojson, from, to) {
        if (from === to) {
            return geojson;
        }
        /**
         * 递归转换坐标数组
         */
        const transformCoordinateArray = (coords) => {
        ...
        const features = geojson.features.map(feature => ({
            ...
            geometry: {
                ...feature.geometry,
                coordinates: transformCoordinateArray(feature.geometry.coordinates),
            },
            properties: {
                ...feature.properties,
