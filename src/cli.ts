...
program
  ...
  .action(async (options) => {
    ...
    const distances = options.distance.split(',').map((d: string) => parseInt(d.trim(), 10));
...
program
  ...
  .action(async (options) => {
    const from = options.from.toUpperCase() as CoordinateSystem;
    const to = options.to.toUpperCase() as CoordinateSystem;

    // 验证坐标系
    const validSystems: CoordinateSystem[] = ['WGS84', 'GCJ02', 'BD09'];
    ...
    // 执行转换
    const transformed = CoordinateTransformer.transformGeoJSON(
      geojson as any,
      from,
      to
    );
