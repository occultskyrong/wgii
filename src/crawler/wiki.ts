...
export class WikiCrawler {
   ...
   * 维基百科国家首都列表页面URL
   */
  private static readonly WIKI_CAPITALS_URL = 'https://en.wikipedia.org/wiki/List_of_national_capitals';
   ...
   * @returns 首都数据映射
   */
  async getCapitals(): Promise<CapitalsData> {
   ...
   * @returns 首都数据映射
   */
  private parseCapitalsTable(html: string): CapitalsData {
   ...
   */
  async saveCapitals(capitals: CapitalsData): Promise<void> {
   ...
   * @returns 排序后的首都数据
   */
  private sortCapitals(capitals: CapitalsData): CapitalsData {
    const sorted: CapitalsData = {};
