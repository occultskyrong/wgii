...
export class Sparse {
  ...
  static simplifyGeoJSONCoordinates(
    ...
    switch (type) {
      case 'Point':
        // 点类型无需简化
        return { type, coordinates } as GeoJSONGeometry;

      case 'LineString':
        // 线类型，直接简化
        return {
          type,
          coordinates: this.simplify(coordinates as Coordinate[], tolerance),
        } as GeoJSONGeometry;
      ...
      case 'Polygon':
        ...
        return {
          type,
          coordinates: (coordinates as Coordinate[][]).map((ring) =>
      ...
      case 'MultiLineString':
        ...
        return {
          type,
          coordinates: (coordinates as Coordinate[][]).map((line) =>
            this.simplify(line, tolerance)
          ),
        } as GeoJSONGeometry;
      ...
      case 'MultiPolygon':
        ...
        return {
          type,
          coordinates: (coordinates as Coordinate[][][]).map((polygon) =>
            polygon.map((ring) => this.simplifyPolygonRing(ring, tolerance))
          ),
        } as GeoJSONGeometry;
   ...
   */
  private static simplifyPolygonRing(ring: Coordinate[], tolerance: number): Coordinate[] {
    if (ring.length <= 3) {
      return ring.slice();
    }
